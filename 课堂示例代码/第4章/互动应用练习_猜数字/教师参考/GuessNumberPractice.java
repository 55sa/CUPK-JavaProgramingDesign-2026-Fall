...
// 教师参考答案 / Teacher answer key
// RU: Замените только __TODOxx__, остальной код оставьте без изменений.
// TM: Diňe __TODOxx__ belliklerini çalşyň; galan kody saklaň.
...
// TM: Oýun klasy düzgünleri we ýagdaýy, esasy klas bolsa girişi we çykyşy dolandyrýar.
...
class GuessGame {
    ...
    // TODO09：返回已用次数的字段。
    // TM: Ulanylan synanyşyklaryň sanyny saklaýan meýdany gaýtaryň.
    ...
    int getRemaining() {
        ...
        // TM: Galan synanyşyklar: iň köp synanyşyk sanyndan ulanylan sany aýryň.
    // RU: Оставшиеся попытки равны лимиту минус число использованных попыток.
        return maxAttempts - attempts;
    }
    // TODO11：返回胜利状态字段。
    // TM: Ýeňiş ýagdaýyny saklaýan meýdany gaýtaryň.
    ...
    boolean isOver() {
        ...
        // TM: Ýeňiş gazanylanda ÝA-DA synanyşyklar gutaranda oýun tamamlanýar.
    ...
    String getSummary() {
        if (won) return "猜中了！/ Вы победили! / Ýeňdiňiz! / You won! 猜测次数 / Попытки: / Synanyşyklar: / Attempts: " + attempts;
        if (isOver()) return "次数用完！/ Попытки закончились! / Synanyşyklar gutardy! / No attempts left! 答案 / Ответ: / Jogap: / Answer: " + secret;
        return "游戏已退出。/ Игра остановлена. / Oýun bes edildi. / Game stopped. 已用次数 / Использовано попыток: / Ulanylan synanyşyklar: / Attempts used: " + attempts;
    }
}
...
public class GuessNumberPractice {
    ...
    // TM: Taýýar giriş metody: main-i işlediň; bu ýerde boşluk ýok.
    ...
    // TM: main bu metody çagyrýar; işletmezden öň ähli TODO-lary dolduryň.
    ...
    private static void play(String[] args) {
        ...
        if (secret < 1 || secret > 20) {
            System.out.println("固定答案必须在 1–20 内。/ Фиксированный ответ должен быть от 1 до 20. / Bellenen jogap 1–20 aralygynda bolmaly. / The fixed answer must be within 1–20.");
            return;
        }
        // TODO17：创建一个 GuessGame 对象，传入 secret 和 5 次机会。
        // TM: secret we 5 argumentleri bilen GuessGame obýektini dörediň.
    // RU: Создайте объект GuessGame с аргументами secret и 5.
        // Create a GuessGame object using secret and a limit of 5 attempts.
        GuessGame game = new GuessGame(secret, 5);
        // TODO18：创建读取标准输入的 Scanner 对象。
        // TM: Standart girişden okaýan Scanner obýektini dörediň.
        // RU: Создайте объект Scanner для чтения из стандартного ввода.
        // EN: Create a Scanner object that reads standard input.
        Scanner input = new Scanner(System.in);
        System.out.println("=== 猜数字 / Угадай число / Sany tap / Guess the Number ===");
        System.out.println("答案为 1–20 的整数，你有 5 次机会。/ Угадайте целое число от 1 до 20 за 5 попыток. / 1–20 aralygyndaky bitin sany 5 synanyşykda tapyň. / Guess an integer from 1 to 20 in 5 attempts.");
        System.out.println("输入 0 退出；无效输入不扣次数。/ Введите 0 для выхода; неверный ввод не расходует попытки. / Çykmak üçin 0 giriziň; nädogry giriş synanyşyklary azaltmaýar. / Enter 0 to quit; invalid input costs no attempt.");

        // TODO19：调用判断游戏是否结束的方法，填写方法名。
        // TM: Oýnuň gutarandygyny barlaýan metodyň adyny ýazyň.
    // RU: Укажите имя метода, проверяющего завершение игры.
        // Fill in the method name that checks whether the game is over.
        while (!game.isOver()) {
            System.out.println("剩余次数 / Осталось попыток: / Galan synanyşyklar: / Attempts left: " + game.getRemaining());
            ...
            System.out.println("INPUT> 请输入整数 / Введите целое число: / Bitin san giriziň: / Enter an integer:");
            if (!input.hasNextLine()) break;
            // TODO20：填写读取一整行的方法名。
            // TM: Tutuş setiri okaýan metodyň adyny ýazyň.
            ...
            try {
                ...
                System.out.println("请输入整数。/ Пожалуйста, введите целое число. / Bitin san giriziň. / Please enter an integer.");
                continue;
            }
            // TODO21：输入 0 时退出当前游戏循环。
            // TM: 0 girizilende oýun siklinden çykyň.
            // RU: При вводе 0 выйдите из игрового цикла.
            // EN: Exit the game loop when the player enters 0.
            if (number == 0) break;
            // TODO22：小于 1 或大于 20 都是无效输入。
            // TM: 1-den kiçi ÝA-DA 20-den uly sanlar nädogrudyr.
            // RU: Числа меньше 1 или больше 20 недопустимы.
            // EN: A number below 1 or above 20 is invalid.
            if (number < 1 || number > 20) {
                System.out.println("范围错误，请输入 1–20。/ Вне диапазона; введите число от 1 до 20. / Aralykdan daşarda; 1–20 aralygynda san giriziň. / Out of range; enter 1–20.");
                continue;
            }
            // TODO23：调用 game 的 guess 方法，传入 number，获得判断结果。
            // TM: game obýektiniň guess metodyny number argumenti bilen çagyryň.
    // RU: Вызовите метод guess объекта game с аргументом number.
            ...
            if (result < 0) System.out.println("太小了！/ Слишком мало! / Örän kiçi! / Too low!");
            else if (result > 0) System.out.println("太大了！/ Слишком много! / Örän uly! / Too high!");
            else System.out.println("正确！/ Верно! / Dogry! / Correct!");
        }
        // TODO24：调用总结方法，填写方法名。 / Fill in the summary method name.
        // TM: Oýnuň jemini gaýtarýan metodyň adyny ýazyň.
